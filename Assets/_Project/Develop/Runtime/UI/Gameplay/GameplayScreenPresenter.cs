...
namespace Assets._Project.Develop.Runtime.UI.Gameplay
{
    public class GameplayScreenPresenter : IPresenter
    {
        ...
        {
            _view.BuildingHealthView.SetText($"Health: {Mathf.RoundToInt(value)}/{Mathf.RoundToInt(_levelConfig.TowerMaxHealth)}");
        }
        ...
        {
            _view.WaveView.SetText($"Wave: {value}/{_stageProviderService.StagesCount}");
        }
        ...
        private void UpdateRest()
        {
            if (_restCycleService.IsResting.Value)
                _view.RestTimerView.SetText($"Rest {_restCycleService.RemainingTime.Value:0}");
            else
                _view.RestTimerView.SetText("Attack");
        }
        ...
        private void UpdateMode(PlayerModes mode)
        {
            switch (mode)
            {
                case PlayerModes.PlaceMine:
                    _view.ModeView.SetText($"Set mines - LMB, cost: {_levelConfig.MineConfig.Cost}");
                    break;

                case PlayerModes.Explode:
                    _view.ModeView.SetText("Explosion mobes - LMB");
                    break;
