...
public class JamperService
{
    ...
    public JamperService(float speed, NavMeshAgent agent, MonoBehaviour coroutineRunner, AnimationCurve yOffsetCurve)
    {
        _speed = speed;
        _agent = agent;
    ...
    public void Jump(OffMeshLinkData offMechLinkData)
    {
        if (InProcess)
            return;

        _jumpProcess = _coroutineRunner.StartCoroutine(JumpProcess(offMechLinkData));
    }
